...
# Use the S3 gateway endpoint for S3 traffic.
...
# Interface endpoints for Secrets Manager, KMS, SSM (+ SSM Messages / EC2 Messages
# for Session Manager) and CloudWatch Logs, so this traffic never has to leave the
# VPC via NAT. Kept behind a flag so the demo stack can skip the extra ENI cost.
...
locals {
  interface_endpoint_services = [
    ...
    "ec2messages",
    "logs",
    "monitoring", # CloudWatch metrics API
  ]
}
